  ...
  return dispatch => {
    ...
    if (username === "" || email === "" || phone === "" || password === "") {
      dispatch({
        ...
        payload: "All form should be filled"
      });
    } else {
      axios
        .get("http://localhost:1971/users", {
          params: {
            username
            // username : username;
          }
        ...
        .then(res => {
          if (res.data.length === 0) {
            axios
              .post("http://localhost:1971/users", {
                username: username,
                email: email,
                phone: phone,
                password: password
            ...
            dispatch({
              type: AUTH_SYSTEM_ERROR,
              payload: "Username has been used"
            });
          }
        })
        .catch(err => {
...
export const select_item = selectedItem => {
  return {
    type: SELECT_ITEM,
